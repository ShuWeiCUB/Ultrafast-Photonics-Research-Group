...
# logger
hp['BM_nt_epochs'] = 2000
hp["BM_log_frequency"] = 100
hp['BM_log_checkpoints'] = True
hp["BM_log_checkpoint_freq"] = 100
...
class PI_FROG(NeuralNetwork):
    ...
    def fit(self, t, FROG0, FROG1):
        ...
        def loss_and_flat_grad(w,epoch):
            ...
            grad = [] # Must save as a list
            grad_flat = [] # Must save as a list
...
########################## Base Model Loss ###################################
    ...
    def load_latest_checkpoint(self, indexnum = 'last',basemodel = False):
        ...
        if basemodel:
            ...
            print(self.logger.logger_data.keys())
            
            print(self.logger.logger_data['N2'])
            np.save("N2_values.npy", self.logger.logger_data['N2'])
            #np.save("N2_values.npy", self.logger.logger_data['N2'])
            
            print(self.logger.logger_data['loss_log'])
            np.save("loss_values.npy", self.logger.logger_data['loss_log'])

            print(self.logger.logger_data['epoch_ax'])
            ...
            if indexnum == 'last':
                ...
                inx = int(indexnum/self.logger.log_checkpoint_freq - 1)
...
def get_PINN(hp = hp, datafname = 'PI_FROG_SIM_v1.mat',indexnum = 'last',ModelDirectory = os.getcwd(),D_trainable = False, N2_trainable = False, Load_Trained_model = False):
    ...
    FROG_0_skimg = resize(FROG_0, (26, 64), order=3, preserve_range=True, anti_aliasing=True)
    FROG_1_skimg = resize(FROG_1, (26, 64), order=3, preserve_range=True, anti_aliasing=True)
    ...
    time_axis = FROG_0_content['t_exp'].flatten() #size 128
    ...
    plt.plot(t_span, np.real(E_real_0)[0].squeeze(), 'o-', label='Re(E0_interp) vs linspace', alpha=0.7)
    plt.xlabel("Time (fs)")
    plt.ylabel("Re(E0)")
    plt.title("Re(E0_interp) vs linspace(time_axis)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('E0real_vs_linspace.png')
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('NN_hp[u_0]_vs_NNgrid.png')
    plt.close()
