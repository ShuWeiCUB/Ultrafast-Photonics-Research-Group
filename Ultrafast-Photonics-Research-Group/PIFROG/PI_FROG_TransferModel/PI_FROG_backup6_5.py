...
class PI_FROG(NeuralNetwork):
    ...
    def loss(self, t, q, FROG_0, FROG_1, all_h0r, all_h0i, all_h1r, all_h1i, epoch, phase_loss_shared, frog_loss_shared, bound_loss_shared):
        ...
        for i in range(0,self.RKsteps):
            ...
            #all_h0 = tf.complex(tf.convert_to_tensor(all_h0r, dtype=tf.float64),tf.convert_to_tensor(all_h0i, dtype=tf.float64))
            #all_h1 = tf.complex(tf.convert_to_tensor(all_h1r, dtype=tf.float64),tf.convert_to_tensor(all_h1i, dtype=tf.float64))
            ...
            all_h0r_tensor = tf.convert_to_tensor(all_h0r_np, dtype=tf.float64)
            all_h0i_tensor = tf.convert_to_tensor(all_h0i_np, dtype=tf.float64)
            all_h1r_tensor = tf.convert_to_tensor(all_h1r_np, dtype=tf.float64)
            all_h1i_tensor = tf.convert_to_tensor(all_h1i_np, dtype=tf.float64)

            all_h0 = tf.complex(all_h0r_tensor , all_h0i_tensor)
            all_h1 = tf.complex(all_h1r_tensor ,all_h1i_tensor)
            ...
            '''
                if q != 99 and q != 0:
                    ...
                    updateh0 = tf.reduce_mean(tf.abs(tf.math.angle(h0) - tf.math.angle(all_h0[q+1]))) 
                    updateh1 = tf.reduce_mean(tf.abs(tf.math.angle(h1) - tf.math.angle(all_h1[q+1]))) 
                    
                    if updateh0 >= 0.0:
                        phase_penalty += tf.reduce_mean(tf.abs(tf.math.angle(h0) - tf.math.angle(all_h0[q+1])))
                        #phase_penalty += tf.reduce_mean(tf.abs(tf.math.angle(h0) - tf.math.angle(all_h0[q-1]))) 
                    
                    if updateh1 >= 0.0:
                        phase_penalty += tf.reduce_mean(tf.abs(tf.math.angle(h1) - tf.math.angle(all_h1[q+1])))
                        #phase_penalty += tf.reduce_mean(tf.abs(tf.math.angle(h0) - tf.math.angle(all_h0[q-1]))) 
                if q == 99:

                    updateh0 =  tf.reduce_mean(tf.square(tf.math.angle(h0) - tf.math.angle(all_h0[q-1]))) 
                    updateh1 =  tf.reduce_mean(tf.square(tf.math.angle(h1) - tf.math.angle(all_h1[q-1])))
            ...
            hlength = int(tf.size(h0).numpy())
            
            for a in range(hlength):
                all_h0r[a + q*hlength] = tf.math.real(h0[a])
                all_h0i[a + q*hlength] = tf.math.imag(h0[a])    
                all_h1r[a + q*hlength] = tf.math.real(h1[a])
                all_h1i[a + q*hlength] = tf.math.imag(h1[a])

            

            

            #all_h0r[q] = tf.math.real(h0)
            #all_h0i[q] = tf.math.imag(h0)
            #all_h1r[q] = tf.math.real(h1)
            #all_h1i[q] = tf.math.imag(h1)
        ...
        n = 5
        #boundary_penalty = tf.reduce_mean(tf.square(h0[:n])) + tf.reduce_mean(tf.square(h0[-n:]))
        #boundary_penalty += tf.reduce_mean(tf.square(h1[:n])) + tf.reduce_mean(tf.square(h1[-n:]))

        # Use tf.math.real if your h0/h1 might be complex
        boundary_penalty = tf.reduce_mean(tf.square(tf.math.real(h0[0]))) + tf.reduce_mean(tf.square(tf.math.real(h0[-1])))
        boundary_penalty += tf.reduce_mean(tf.square(tf.math.real(h1[0]))) + tf.reduce_mean(tf.square(tf.math.real(h1[-1])))
        ...
        phase_loss_shared[q] = phase_penalty
        frog_loss_shared[q] = mse0 + mse1
        bound_loss_shared[q] = boundary_penalty
        ...
        def smooth_weight(phase_penalty):
            ...
            offset = 2.0  # Center of the transition
            max_weight = 0.001
        ...
        return mse0 + mse1 + weight*phase_penalty+ 100*boundary_penalty
    ...
    def fit(self, t, FROG0, FROG1, qvalue, grad, grad_flat, counter, weights, loss_value, all_h0r, all_h0i, all_h1r, all_h1i, phase_loss_shared, frog_loss_shared, bound_loss_shared, barrier, lock):
        ...
        def loss_and_flat_grad(w, epoch, grad, grad_flat, counter,weights, loss_value, barrier, lock):
            ...
            for cord in np.atleast_1d(qvalue):
                ...
                with tf.GradientTape() as tape:
                    ...
                    loss_value_q = self.loss(t,cord, FROG_0, FROG_1, all_h0r, all_h0i, all_h1r, all_h1i, epoch, phase_loss_shared, frog_loss_shared, bound_loss_shared)
            ...
            if(cord == 99):
                ...
                for i in range(100):
                    ...
                    totalboundloss += bound_loss_shared[i]
                    

                
                print("phase loss " + str(totalphaseloss))
                print("frog loss " + str(totalfrogloss))
                print("bound loss " + str(totalboundloss))
...
def get_PINN(hp = hp, datafname = 'PINN_FROG_model.mat',indexnum = 'last',ModelDirectory = os.getcwd(),D_trainable = False, N2_trainable = False, Load_Trained_model = False):
    ...
    print("Dispersion sim" + str((sim_hp['D']/2)))
    print("N2 sim " + str((sim_hp['N2'])))
    ...
    print("Dispersion BM" + str((sim_hpBM['D']/2)))
    print("N2 BM " + str((sim_hpBM['N2'])))
    ...
    def Start_PINN_fit(qvalue, grad, grad_flat, counter, weights, loss_value, all_h0r, all_h0i, all_h1r, all_h1i, phase_loss_shared, frog_loss_shared, bound_loss_shared, barrier, lock, loadbasemodel = False):
        if loadbasemodel: 
            pinn.load_latest_checkpoint(basemodel = True)
            
        pinn.fit(NN_hp['t'],\
                 NN_hp['FROG_0'],\
                  NN_hp['FROG_1'], qvalue, grad, grad_flat, counter, weights, loss_value, all_h0r, all_h0i, all_h1r, all_h1i, phase_loss_shared, frog_loss_shared, bound_loss_shared, barrier, lock)
