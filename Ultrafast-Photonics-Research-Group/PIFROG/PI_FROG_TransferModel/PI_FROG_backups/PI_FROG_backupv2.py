...
class PI_FROG(NeuralNetwork):
    ...
    def loss(self, t, q, FROG_0, FROG_1):
        ...
        for i in range(0,self.RKsteps):
            ...
            # Anchor phase using Euler's formula (more numerically stable)
            #angle0 = tf.cast(tf.math.angle(h0[0]), tf.float64)
            #h0 = h0 * tf.complex(tf.cos(angle0), -tf.sin(angle0))
            
            #angle1 = tf.cast(tf.math.angle(h1[0]), tf.float64)
            #h1 = h1 * tf.complex(tf.cos(angle1), -tf.sin(angle1))

            # === MODIFIED PHASE ANCHORING ===
            # Force phase at first point to be 0 for all q
            #target_phase0 = 0.0
            #target_phase1 = 0.0

            target_phase0 = tf.constant(0.0, dtype=tf.float64)
            target_phase1 = tf.constant(0.0, dtype=tf.float64)
            
            # Compute needed phase correction
            phase_correction0 = tf.math.angle(h0[0]) - target_phase0
            phase_correction1 = tf.math.angle(h1[0]) - target_phase1
            
            # Apply correction (using complex exponential for stability)
            h0 = h0 * tf.exp(tf.complex(tf.constant(0.0, dtype=tf.float64), -phase_correction0))
            h1 = h1 * tf.exp(tf.complex(tf.constant(0.0, dtype=tf.float64), -phase_correction1))
            # ===============================
        ...
        if q == 99:
            print(f"FROG loss: {mse0+mse1}, Phase loss: {phase_loss0+phase_loss1}")   
    ...
    def fit(self, t, FROG0, FROG1, qvalue, grad, grad_flat, counter, weights, loss_value, barrier, lock):
...
def get_PINN(hp = hp, datafname = 'PI_FROG_SIM_v1.mat',indexnum = 'last',ModelDirectory = os.getcwd(),D_trainable = False, N2_trainable = False, Load_Trained_model = False):
    ...
    def Start_PINN_fit(qvalue, grad, grad_flat, counter, weights, loss_value, barrier, lock, loadbasemodel = False):
        if loadbasemodel: 
            pinn.load_latest_checkpoint(basemodel = True)
            
        pinn.fit(NN_hp['t'],\
                 NN_hp['FROG_0'],\
                  NN_hp['FROG_1'], qvalue, grad, grad_flat, counter, weights, loss_value, barrier, lock)
