...
T0 = 200;          % pulse width in fs
lam0 = 1550;       % in nmC:\Users\ECEE1B79\OneDrive - UCB-O365\G\Publications\In progress\2024 - MultiDomain-PINN for forward problems in ultrafast nonlinear photonics
s = lam0/(2*pi*3e2*T0)*0;  % self-steepening
RamOn = 0;

distance = pi/4;   % normalized to dispersion length
N = 0.7;          % soliton order
...
nt = 2^8;
Tmax = 5;       % FFT points and window size
...
fname = 'PI_FROG_SIM_v1_BaseModel.mat';
save(strcat('data/',fname),'data','params')

fprintf('\n FILE SAVED IN data FOLDER AS: \n')
fprintf('\n')
fprintf(fname)
fprintf('\n')
